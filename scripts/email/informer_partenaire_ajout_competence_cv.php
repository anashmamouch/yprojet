<?php  function INFORMER_PARTENAIRE_AJOUT_COMPETENCE($connection_database2, $id_affiliate_qui_recommande, $id_affiliate_cible, $competence_texte, $serveur )
{      include('scripts/config.php');  

	 // REQUETTE DE REMPLISSAGE DU MAIL 
     mysql_query('SET NAMES utf8');
	 $id_affiliate  = $id_affiliate_cible; 
     $result        = mysql_fetch_array(mysql_query(" SELECT af.first_name, af.last_name, af.zip_code, af.email, af.city, af.id_affiliate, aa.password 
	                                                  FROM affiliate_details af, affiliate aa WHERE af.id_affiliate=aa.id_affiliate AND af.id_affiliate =".$id_affiliate."   ")) or die("Requete pas comprise - #AZSLP31! ");
	 $mail          = $result['email'];
     ...
     $result2                      = mysql_fetch_array(mysql_query(" SELECT af.first_name, af.last_name, af.zip_code, af.email, af.city, af.id_affiliate, aa.password 
	                                                                 FROM affiliate_details af, affiliate aa WHERE af.id_affiliate=aa.id_affiliate AND af.id_affiliate =".$id_affiliate_qui_recommande."   ")) or die("Requete pas comprise - #AZSLP31! ");
	 $mail_qui_recommande          = $result2['email'];
     $first_name_qui_recommande    = $result2['first_name'];
	 $last_name_qui_recommande     = $result2['last_name'];
	 $zip_code_qui_recommande      = $result2['zip_code'];
	 $city_qui_recommande          = $result2['city'];	 
						...
						<table width='600' border='0' cellpadding='0' cellspacing='0' align='left' style='border-collapse:collapse; mso-table-lspace:0pt; mso-table-rspace:0pt;' class='fullImage'>
							...
							<tr>
								<td height='auto' valign='bottom' width='100%' style='text-align: center; font-family: Helvetica, Arial, sans-serif; color: rgb(255, 255, 255); font-size: 13px; line-height: 20px; font-weight: normal;'class='textCenter' id='box2' >
									...
									L'affilié $first_name_qui_recommande $last_name_qui_recommande de $city_qui_recommande <b>souhaite vous recommander pour votre compétence :</b> 
									<br/>
									<i> \"$competence_texte\"<i> 
									<br/>
									<br/>
                                    Afin de la rendre visible de tous,<b>VALIDEZ</b> cette compétence dans votre intranet NosRezo !  
                                    <br/>
									</span><!--<![endif]-->
								</td>
							</tr>
...
//========== Définition du sujet.
$sujet = "Un affilié NosRezo souhaite recommander vos compétences ! ";
//========= 
  
//========== Création du header de l'e-mail.
$header = "From: \"NosRezo.com\"<contact@nosrezo.com>".$passage_ligne;
$header.= "Cc:".$mail_qui_recommande.", contact@nosrezo.com ".$passage_ligne;
